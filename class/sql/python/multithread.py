...
# from oldSharder import sharder, Frame
...
from gvision import process_video_file
import glob
...
statusMeetingAbandoned="MeetingAbandoned"
...
class endpointSharder(threading.Thread):
    ...
    def __init__(self, name, queue=None): 
        ...
        self.__enable_speechhints__ = os.getenv('ENABLE_SPEECH_HINTS', "false") == "true"
    ...
    def process_message(self, body):  
        try:
            ...
            if record["Records"][0]["eventSource"] == "aws:s3":
                ...
                if not self.downloadFile(bucket, key, meetingId, speechhints):
                ...
                if "amazonaws.com" in url:
                    try:
                        ...
                        key = m.group(2)
                        if not self.downloadFile(bucket, key, meetingId, speechhints):
                            LOG.warning({"message": "failed to download from s3 bucket %s, key %s for meeting" % (bucket, key, meetingId)})
                            raise Exception ("Failed to download file from s3, terminate %s" % meetingId)
                    except Exception as ex:
                        #as the second choice, download it as public URL anyway.
                        if not self.downloadFromUrl(url, "", meetingId, speechhints):
                    ...
                    if not self.downloadFromUrl(url, "", meetingId, speechhints):
                        LOG.error({"message": "failed to download from URL %s for meeting %s" %(url, meetingId,)})
            ...
            sts = self.__oldSharder__.shard(path=AUDIO_FOLDER, meetingId=meetingId, \
                trainable=trainable, ensemble=ensemble, \
                    start_at_utc=start_at_utc, source=source,\
                         language=language, sharder=sharder, speechhints=speechhints)
            ...
            LOG.error({"message":"Exception when process message %s" % str(ex), "VOICEA_MEETING_ID": meetingId, "stack trace": traceback.format_exc()})
            failed_sts = ({  "meetingid": meetingId, \
                'status': statusMeetingAbandoned, \
                'hang_up_reason': str(ex)})  
            self.__wrapi__.post_to_wrapi(meeting_id=meetingId, sts=failed_sts)
            success = False
        return success
    
    def getFileType(self, url, contentType):
        try:
            fType =  content2FileType[contentType]
        except Exception as ex:
            LOG.warning({"message": "content type %s is not supported. assume to mp4? url=%s" % (contentType, url) })
            #use url to decide the file type
            fType = url.split("?")[0].split(".")[-1]
            # fType = "flac"
            if len(fType) > 4:
                fType = None
        return fType
    
    def downloadBinary(self, url, meetingId, fileType):
        originFile = AUDIO_FOLDER + meetingId + ".orig." + fileType
        for i in range(3):
            try:
                response = requests.get(url, allow_redirects=True)
                if response.status_code != 200 :
                    LOG.error({"message":"error occurred getting file from URL: %s on try# %i of 3"% (response.reason, i),  "VOICEA_MEETING_ID": meetingId, })
                else:
                    downloaded = True
                    break
            except Exception as ex:
                LOG.warning({"message":"error occurred getting file to from URL: %s on try# %i of 3, excpetion: %s "% (response.reason, i, ex),  "VOICEA_MEETING_ID": meetingId, })
            
                # raise Exception("failed download status code %s" % response.status_code)
            if downloaded: 
                LOG.info({"message":"download file to local file from Url", "VOICEA_MEETING_ID": meetingId, "url":url, "file": originFile,})
                file_handle = open(originFile, "wb")
        ...
        if os.path.exists(originFile):
            raise Exception("failed download file after three tries, %s" % url)

        return originFile       
    
    
    def downloadFromUrl(self, url, token, meetingId, speechhints=None):
        success = False
        downloaded = False
        try:
            ...
            if downloaded:
                ...
                #specific to vidcast, most of time, we are expecting the mp4 or wav file directly. 
                #ssume vidcast:
                if "application/json" == contentType:
                    try:
                        videourl = response.json().get("screen_asset_url")
                        audiourl = response.json().get("audio_asset_url")
                        if videourl is not None and audiourl is not None:
                            audioFile = self.downloadBinary(audiourl, meetingId, "wav")
                            if os.path.exists(audioFile):
                                downloaded = True 
                                success = self.convertFileFormat(meetingId,"wav", audioFile, waveFile)
                    
                            # videoFile = self.downloadBinary(videourl, meetingId, "mp4")
                            # if os.path.exists(videoFile) and self.__enable_speechhints__:
                            # speechhints = self.extractTextFromVideo(videourl, meetingId)
                                # speechhints = []
                            speechhints = process_video_file(videourl, meetingId)
                            # self.downloadFromUrl(videourl, token, meetingId, speechhints)
                    except Exception as ex:
                        raise Exception("cannot support URL: %s. the exception is %s" % (url, ex))
                else:  #hope this the url for the binary video/audio url.
                    fileType = self.getFileType(url,contentType)  
                    if fileType is None:
                        raise Exception("cannot support this URL as audio/video url")                 
                    originFile = AUDIO_FOLDER + meetingId + ".orig." + fileType
                    LOG.info({"message":"download file to local file from Url", "VOICEA_MEETING_ID": meetingId, "url":url, "file": originFile,})
                    file_handle = open(originFile, "wb")
                    file_handle.write(response.content)
                    ...
                    file_handle.seek(0)
                    downloaded = True 
                    success = self.convertFileFormat(meetingId,fileType, originFile, waveFile, speechhints)
                    
            else:
                raise Exception("after three tries, failed to download from URL") 
            
        except Exception as ex:
            LOG.warning({"message":"failed to download file or convert file: for VOICEA_MEETING_ID %s,\
                exception: %s" % (meetingId, str(ex)), \
                    "VOICEA_MEETING_ID":meetingId,})
            
        return success
    
    # @measure
    def downloadFile(self, bucket, key, meetingId, speechhints):
        success = False
        try:
            ...
            LOG.warn({"message":"failed to download file from s3 bucket %s: for VOICEA_MEETING_ID %s, \
                exception: %s" % (meetingId, bucket, str(ex)),\
                    "VOICEA_MEETING_ID": meetingId})

        return success
    
    def extractAudioFromVideo(self, meetingId, fileType, originFile):
        outputFile = "".join(originFile.split(".")[:-1]) + ".wav"
        try:
            LOG.info({"message":"Run ffmpeg to extract audio", "fileType": fileType, "originFile":originFile,} )
            os.system("ffmpeg -i %s -y -sn -vn %s >/dev/null 2>&1" % (originFile, outputFile) )
            if not (os.path.exists(outputFile)):
                raise Exception("audio file not extracted")
        except Exception as ex:
            LOG.error({"message": "trying to extract Audio from %s and failed. Exception is %s" % (fileType, ex), \
                "VOICEA_MEETING_ID": meetingId,} )
            return originFile
        return outputFile
        
    def convertFileFormat(self, meetingId, fileType, originFile, waveFile, speechhints=None):
        
        if fileType in ["mp4", "mov", "wmv", "m3u8", "ts", "avi", "3gp", "m4a", ]:
            if self.__enable_speechhints__:
                speechhints = self.extractTextFromVideo(originFile, meetingId)
            audioFile = self.extractAudioFromVideo(meetingId, fileType, originFile) 
        else:
            audioFile = originFile
        success = True
        sampleRate = self.getSampleRate(audioFile)
        # LOG.info({"message":"Convert file", "wavefile": waveFile, "sampleRate=":sampleRate, "originFile":originFile,} )
        #remove if not (os.path.isfile(waveFile) and sampleRate == DEFAULT_SAMPLE_RATE):
        os.system("sox --clobber %s -r %s -c 1 -b 16 %s > /dev/null 2>&1" % (audioFile, DEFAULT_SAMPLE_RATE, waveFile) )
        if not (os.path.exists(waveFile)):
            success = False
        LOG.info({"message":"Convert file", "wavefile": waveFile, "sampleRate=":sampleRate, "originFile":audioFile, \
            "result success": success, "VOICEA_MEETING_ID": meetingId} )
        
        return success
    
    def extractTextFromVideo(self, originFile, meetingId):
        try:
            speechhints = []
            results = process_video_file(originFile, meetingId)
            for result in results:
                (_, sh) = result
                for x in sh: 
                    speechhints.append(x) 
            # print("processed video file and got speech hints:\n", speechhints)
            LOG.info({"message", "speechhints from video: %s" % speechhints})
            sh = set(speechhints).sort()
            return sh
        except Exception as ex: 
            LOG.warning({"message": "did not get speechhints exception: %s" % (ex)})
            return ["Cisco", ]
...
def testEndPointSharder():
    e = endpointSharder("test1")
    message = '{"Records": [{"eventSource": "asr", "meetingid":"439e87e2-44a3-4885-9988-bea60e1e2d0", "uri":"https://webex-ai-meeting-assets-cint-uscentral1-0.s3.us-west-2.amazonaws.com/00/00/00/00000000/audio/vidcast_test_audio.mp4?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIA2GBUB2VMJBGSY7W3%2F20210811%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20210811T175948Z&X-Amz-Expires=7200&X-Amz-SignedHeaders=host&X-Amz-Signature=8113b534cfe06acd3908d9f22261b2268bbfb292938953f5a45632c453e32fc2" }]}'
